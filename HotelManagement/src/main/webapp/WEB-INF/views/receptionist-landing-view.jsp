...
<body>
	...
	<input style="background-color: inherit; border: bold; font-size: 18px;" type="submit" value="LOG OUT">
	</form>
</div>
	...
	<div>
		...
		<div style="text-align: center;">
			<form action="receptionistRequest.htm" method="get">
				<h3>Please choose the task you want to perform today!</h3>
				<select name="taskDD" id="taskTable">

					<option value="New Booking">Make a new booking.</option>
					<option value="Raise Request">Raise a request for existing booking.</option>
					<option value="Checkout">Checkout customer.</option>

				</select> <input style="color: #333;" class='scoreUpdate' type="submit"
					value="Submit.">
		...
		<div style="text-align: center;">
			<h3>Current Bookings Available.</h3>
			<table class="rwd-table" width="70%" align='center'>
				...
				<tbody>
					<c:forEach items="${currentBookings}" var="booking">
						<tr>
							<td data-th="Booking ID"><c:out
									value="${booking.getBookingID()}" /></td>
							<td data-th="Room Number"><c:out
									value="${booking.getRoomNumber()}" /></td>
							<td data-th="Check-In Date"><c:out
									value="${booking.getCheckinDate()}" /></td>
							<td data-th="Check-Out Date">
							<c:out
									value="${booking.getCheckoutDate()}" />
							</td>
							<td data-th="Total Bill"><c:out
									value="${booking.getTotalBill()}" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			...
			<table class="rwd-table" width="70%" align='center'>
				...
				<tbody>
					<c:forEach items="${openRequests}" var="request">
						<tr>
							<td data-th="Booking ID"><c:out
									value="${request.getRequestType()}" /></td>
							<td data-th="Room Number"><c:out
									value="${request.getRoomNumber()}" /></td>
							<td data-th="Check-In Date"><c:out
									value="${request.getRequestTime()}" /></td>
